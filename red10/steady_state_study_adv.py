...
from config_ethem import evad, elntd, cryo, bias, load
...
rt_list = [elntd.resistivity.subs(evad).subs({'T_ntd':t}) for t in t_range]
plt.figure('R(T)')
...
#        sol = solve_ss(evad, v, tb, x0=XO, printsuccess=False)
        ...
        r_elntd = elntd.resistivity.subs(ss_dict).subs(evad)
        r_list.append(r_elntd)

        sens_elntd_sy = (elntd.current * elntd.resistivity.diff(elntd.temperature))
        sens_elntd = sens_elntd_sy.subs(ss_dict).subs(evad)
        sens_list.append(sens_elntd)
