...
v_array = 10**np.linspace(np.log10(0.02), np.log10(50), 10)

#t_array = np.linspace(0.018, 0.020, 3)
##v_array = 10**np.linspace(np.log10(0.01), np.log10(10), 10)
#v_array = np.array([2.])
...
c_list = list()
for i in np.linspace(0.05, 0.95, len(t_array)):
    c_list.append(cmap(i))
...
ni_pulse_fun = eth.num_int_param(param, evad, fs, L)
...
##        ni_pulse_array = ni_pulse_fun(p_eval, sol.x)
##
##        plt.plot(time_array, ni_pulse_array[-1])
##
##        ni_pulse_list.append(ni_pulse_array)
#
#    iv_dict[temp] = iv_list
##    ni_pulse_dict[temp] = ni_pulse_list
#
#
#
#
...
#temp_list.sort()
#for c, temp in zip(c_list, temp_list):
#    plt.loglog(v_array, iv_dict[temp], label='{0:.4f} K'.format(temp), color=c)
#
#plt.grid(True)
...
#fig = plt.figure()
#per_list = list()
#A = eth.per_param(param, evad)
#for temp in t_array:
#    B = A((temp, 2.))(time_array)
#
#    plt.plot(time_array, B[:,0])
#
##per_num = list((eth.System.perturbation.matrix).subs(evad))
#
...
##per_fun = eth.per_param(param, evad)((0.018, 2.))
#
#perf_fun = eth.perf_param(param, evad)((0.018, 2.))
##perf_fun = per_fun((0.018, 2.))
freq_array = np.array([0,1,2])

##A = per_fun(time_array)
#B = perf_fun(freq_array)
#
#
#print B
...
##        pulse =
#        plt.plot(time_array, -ni_pulse_dict[temp][i][-1],
#                 label='{0:.4f} K, {1:.3e} V'.format(temp, v))
#
#plt.grid(True)
#fig_pulse.legend(loc='right')
...
#
#
##ni_sol_ss = ss_point((0.018, 2.)).x
##ni_time_array = np.arange(0, L, fs**-1)
##ni_pulse_array = ni_pulse_fun((0.018, 2.), ni_sol_ss)
#
## pulse amplitude
...
#
## fft freq
#fi_pulse_fft = eth.response_event(eth.System.perturbation.matrix, edict, fs)(fi_freq_fft)
