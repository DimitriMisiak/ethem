...
param = (i_bias, t_cryo, t_nbsi)
eqs_fun = sy.lambdify(param, eqs_num, modules="numpy")

t_array = np.linspace(14.e-3, 50e-3, 20)
tn_array = np.linspace(16e-3, 50e-3, 20)
i_array = 10**np.linspace(-11, -9.5, 20)

i_mesh, t_mesh, tn_mesh = np.meshgrid(i_array, t_array, tn_array)
eqs_mesh = eqs_fun(i_mesh, t_mesh, tn_mesh)
...
#sc = ax.scatter(i_mesh, t_mesh, tn_mesh, c=eqs_mesh[0], cmap=plt.hot())
sc = ax.scatter(np.log10(i_mesh.ravel()), t_mesh.ravel(), tn_mesh.ravel(),
                c=eqs_mesh[0].ravel(), cmap=blue_red1,
                norm=colors.SymLogNorm(1e-14, vmin=-3.e-12, vmax=3.e-12)
#                vmin=-3.0e-12,
#                vmax=3.0e-12
                )
fig.colorbar(sc)
ax.set_xlabel('i_bias')
ax.set_ylabel('t_cryo')
ax.set_zlabel('t_nbsi')
